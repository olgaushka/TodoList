...
final class TodoItemScrollView: UIScrollView {
    ...
    override func layoutSubviews() {
        ...
        if isLandscape {
            ...
            self.itemTextView.frame = .init(
                x: Consts.itemTextViewInsets.left,
                y: Consts.itemTextViewInsets.top,
                width: itemTextViewWidth,
            ...
            self.itemTextView.frame = .init(
                ...
                y: Consts.itemTextViewInsets.top,
                width: itemTextViewWidth,
                height: itemTextViewHeight
            )

            self.containerView.frame.origin = .init(
                x: Consts.containerViewInsets.left,
                y: self.itemTextView.frame.maxY + Consts.containerViewInsets.top
            )

            self.importanceLabel.sizeToFit()
            self.importanceLabel.frame.origin = .init(
                x: Consts.importanceLabelInsets.left,
                y: Consts.importanceLabelInsets.top
            )

            self.importanceSegmentedControl.frame.size = .init(
                width: Consts.importanceSegmentedControlSize.width,
                height: Consts.importanceSegmentedControlSize.height
            )

            self.importanceSegmentedControl.frame.origin = .init(
                x: self.containerView.bounds.width - Consts.importanceSegmentedControlInsets.right - self.importanceSegmentedControl.bounds.width, // swiftlint:disable:this line_length
                y: Consts.importanceSegmentedControlInsets.top
            )

            self.dividerView.frame = .init(
                x: Consts.dividerViewInsets.left,
                y: Consts.dividerViewInsets.top,
                width: containerView.bounds.width - Consts.dividerViewInsets.left - Consts.dividerViewInsets.right,
                height: Consts.dividerViewHeight
            )

            self.deadlineLabel.sizeToFit()
            self.deadlineLabel.frame.origin = .init(
                x: Consts.deadlineLabelInsets.left,
                y: self.dividerView.frame.maxY + Consts.deadlineLabelInsets.top
            )

            self.deadlineSwitch.sizeToFit()
            self.deadlineSwitch.frame.origin = .init(
                x: self.containerView.bounds.width - Consts.deadlineSwitchInsets.right - self.deadlineSwitch.frame.width, // swiftlint:disable:this line_length
                y: self.dividerView.frame.maxY + Consts.deadlineSwitchInsets.top
            )
            self.deadlineButton.sizeToFit()
            self.deadlineButton.frame.origin = .init(
                x: Consts.deadlineButtonInsets.left,
            ...
            let datePickerHeight = self.datePicker.sizeThatFits(.init(width: datePickerWidth, height: .greatestFiniteMagnitude)).height // swiftlint:disable:this line_length
            ...
            self.containerView.frame.size = .init(
                width: self.bounds.width - Consts.containerViewInsets.left - Consts.containerViewInsets.right,
                height: containerViewLastElementMaxY
            )

            self.deleteButton.frame.size = .init(
                width: self.bounds.width - Consts.deleteButtonInsets.left - Consts.deleteButtonInsets.right,
                height: Consts.deleteButtonHeight
            )
            self.deleteButton.frame.origin = .init(
                x: Consts.deleteButtonInsets.left,
                y: self.containerView.frame.maxY + Consts.deleteButtonInsets.top
            )
    ...
    private static func makeItemTextView() -> UITextView {
        ...
        textView.textContainerInset = .init(
            top: Consts.itemTextViewInnerInsets.top,
            left: Consts.itemTextViewInnerInsets.left,
            bottom: Consts.itemTextViewInnerInsets.bottom,
            right: Consts.itemTextViewInnerInsets.right)
...
//        textView.textColor = UIColor.lightGray
    ...
    private func setupSubviews() {
        ...
        self.importanceSegmentedControl.addTarget(
            self,
