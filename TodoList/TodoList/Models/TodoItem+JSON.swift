...
extension TodoItem {
    ...
    static func parse(json: Any) -> TodoItem? {
        ...
        if let rawImportance = jsonObject["importance"] {
            guard let importanceString = rawImportance as? String,
                  let importanceValue = Importance(rawValue: importanceString) else { return nil }
        ...
        if let rawDeadline = jsonObject["deadline"] {
            guard let deadlineString = rawDeadline as? String,
                  let deadlineDate = formatter.date(from: deadlineString) else { return nil }
        ...
        if let rawModifiedAt = jsonObject["modifiedAt"] {
            guard let modifiedAtString = rawModifiedAt as? String,
                  let modifiedAtDate = formatter.date(from: modifiedAtString) else { return nil }
